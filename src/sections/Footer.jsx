...
const Footer = () => {
  return (
    <footer className='mac-container '>
      <div className='flex justify-between items-start gap-20 flex-wrap ma-lg:flex-col'>
               ...
               <h4 className='text-white font-montserrat text-2xl leading-normal font-medium mb-6'>{section.title}</h4>
                <ul>
                  {section.links.map((link)=>(
                    <li className='mt-3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer' key={link.key}>
                      <a> {link.name}</a>
                      </li>
                  ))}
                </ul>
